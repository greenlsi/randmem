...
*/
  ...
  int getDetails()
  {
     ...
     //XXX-marina: I don't want to use assembler code, thanks
     /*_cpuida();*/
     ...
     sprintf(configdata[1], "  Solaris Functions"); 
     //XXX-marina Functions not available in Solaris
     /*CPUconf  =  get_nprocs_conf();*/
     /*CPUavail =  get_nprocs();*/
     /*sprintf(configdata[2], "  get_nprocs() - CPUs %d, Configured CPUs %d", CPUconf, CPUavail);*/
     /*pages = get_phys_pages();*/
     /*pagesize = getpagesize();*/
     /*ramGB = ((double)pages * (double)pagesize / 1024 / 1024 / 1024);*/
     /*sprintf(configdata[3], "  get_phys_pages() and size - RAM Size %5.2f GB, Num pages %ld, Page Size %d Bytes", ramGB, pages, pagesize);*/

     //XXX-marina - Translation to Solaris-compatible functions
     CPUconf= sysconf(_SC_NPROCESSORS_CONF); 
     CPUavail= sysconf(_SC_NPROCESSORS_ONLN);
     sprintf(configdata[2], "  get_nprocs() - CPUs %d, Configured CPUs %d", CPUconf, CPUavail);
     ...
     if (uname(&udetails) > -1)
     {
        sprintf(configdata[4], "  uname() - %s, %s, %s", udetails.sysname,
                                  udetails.nodename, udetails.release);  
        sprintf(configdata[5], "  %s, %s", udetails.version, udetails.machine);
     }
     else
     {
         sprintf(configdata[6], "  uname() - No details"); 
         sprintf(configdata[7], "  ");
     }
